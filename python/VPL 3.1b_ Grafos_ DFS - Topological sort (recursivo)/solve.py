...
def dfs_topological_sort(graph):
    ...
    def dfs(visibleNodes, graph, v, order):
        nonlocal N
        visibleNodes.add(v)
        for edges in graph.neighbors(v):
            if edges not in visibleNodes:
                dfs(visibleNodes, graph, edges, order)
    ...
    for v in graph:
        if v not in visibleNodes:
            dfs(visibleNodes, graph, v, order)
