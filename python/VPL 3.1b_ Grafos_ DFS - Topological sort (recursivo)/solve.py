...
def dfs_topological_sort(graph):
    ...
    def dfs(order,graph,v,visibleNodes):
        visibleNodes.add(v)
        for arista in graph.neighbor(v):
            if arista not in visibleNodes:
                dfs(order,graph,arista,visibleNodes)
    ...
    for v in graph:
        if v not in visibleNodes:
            dfs(order,graph,v,visibleNodes)
